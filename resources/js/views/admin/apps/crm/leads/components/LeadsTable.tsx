...
import { leadData, type LeadType } from './data'

const columnHelper = createColumnHelper<LeadType>()

const priceRangeFilterFn: FilterFn<LeadType> = (row, columnId, value) => {
...
const LeadsTable = () => {
  const columns = [
    {
      ...
      maxSize: 45,
      size: 45,
      header: ({ table }: { table: TableType<LeadType> }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<LeadType> }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
    ...
    columnHelper.accessor('createdDate', { header: 'Created' }),
    {
      header: 'Actions',
  ...
  const [data, setData] = useState<LeadType[]>(() => [...leadData])
