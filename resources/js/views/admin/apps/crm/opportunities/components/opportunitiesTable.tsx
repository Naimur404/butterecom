...
import { type ColumnFiltersState, createColumnHelper, getCoreRowModel, getFilteredRowModel, getPaginationRowModel, getSortedRowModel, SortingState, useReactTable } from '@tanstack/react-table'
import clsx from 'clsx'
import { Link } from '@inertiajs/react'
import { useState } from 'react'
import { Card, CardBody, CardFooter, CardHeader, FormSelect } from 'react-bootstrap'
import { opportunityData, type OpportunityType } from './data'
...
const OpportunitiesTable = () => {
  ...
  const [data, setData] = useState<OpportunityType[]>(() => [...opportunityData])
    ...
    <Card>
      <CardHeader className="border-light justify-content-between">
        ...
        <div className="d-flex align-items-center gap-2">
          ...
          <div className="app-search">
            <FormSelect value={(table.getColumn('stage')?.getFilterValue() as string) ?? 'All'} onChange={(e) => table.getColumn('stage')?.setFilterValue(e.target.value === 'All' ? undefined : e.target.value)} className="form-control my-1 my-md-0">
      ...
      <CardBody className="p-0">
        <DataTable<OpportunityType> table={table} emptyMessage="No records found" />
      </CardBody>
