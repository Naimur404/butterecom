...
import { getReviewChartOptions, productReviewData, ProductReviewType, reviewData } from './data'
  ...
  const columns: ColumnDef<ProductReviewType, any>[] = [
    {
      ...
      maxSize: 45,
      size: 45,
      header: ({ table }: { table: TableType<ProductReviewType> }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14" checked={table.getIsAllRowsSelected()} onChange={table.getToggleAllRowsSelectedHandler()} />,
      cell: ({ row }: { row: TableRow<ProductReviewType> }) => <input type="checkbox" className="form-check-input form-check-input-light fs-14" checked={row.getIsSelected()} onChange={row.getToggleSelectedHandler()} />,
      enableSorting: false,
    ...
    columnHelper.accessor('product', {
      ...
      cell: ({ row }) => (
        <div className="d-flex align-items-center">
          ...
          <div>
            <h5 className="mb-0">
              <Link href="/apps/ecommerce/products" className="link-reset">
              ...
              <Col xl={5}>
                <div className="p-3">
                  {reviewData.map((review, idx) => {
                    return (
                      <div key={idx} className={clsx('d-flex align-items-center gap-2', { 'mb-2': idx != reviewData.length - 1 })}>
