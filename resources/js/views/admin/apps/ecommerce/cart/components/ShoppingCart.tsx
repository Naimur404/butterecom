  ...
  return (
    <Card>
      <CardHeader className="d-flex justify-content-between align-items-center">
        ...
        <Link href="" className="text-decoration-underline link-offset-2 fw-medium">
          Clear cart
        </Link>
      </CardHeader>
      ...
      <Card.Body>
            ...
            <tbody>
              {cartItem.map((item) => (
                <tr key={item.id}>
                  ...
                  <td>
                    <Link href="" className="text-muted" aria-label={`Remove ${item.name} from cart`} onClick={() => handleRemove(item.id)}>
        ...
        <div className="mt-4">
          <Link href="/apps/ecommerce/products-grid" className="fw-medium d-inline-flex align-items-center gap-1">
