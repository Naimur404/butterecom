...
const skipped = (ctx: ScriptableLineSegmentContext, value: number[] | string) => (ctx.p0.skip || ctx.p1.skip ? value : undefined)
const down = (ctx: ScriptableLineSegmentContext, value: number[] | string) => (ctx.p0.parsed.y && ctx.p1.parsed.y && ctx.p0.parsed.y > ctx.p1.parsed.y ? value : undefined)
    ...
    datasets: [
      {
        ...
        spanGaps: true,
        segment: {
          borderColor: (ctx: ScriptableLineSegmentContext) => skipped(ctx, getColor('chart-dark-rgb', 0.2)) || down(ctx, getColor('danger')),
          borderDash: (ctx: ScriptableLineSegmentContext) => skipped(ctx, [3, 6]),
        },
      },
    ],
  },
